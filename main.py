...
@app.get("/schema")
def get_schema():
    # For the built-in viewer tooling
    from schemas import (
...
# Auth (very basic placeholder token approach)
...
def signup(payload: AuthRequest):
    ...
    user = UserSchema(email=payload.email, password_hash="-", name=payload.name, role=payload.role)
...
def create_influencer_profile(profile: InfluencerProfile):
...
def list_influencers(
    ...
    if min_budget is not None or max_budget is not None:
        # Find profiles where [budget_min, budget_max] overlaps filter range
        budget_filters = []
...
def list_influencer_bookings(influencer_id: str):
    if not ObjectId.is_valid(influencer_id):
        raise HTTPException(status_code=400, detail="Invalid id")
